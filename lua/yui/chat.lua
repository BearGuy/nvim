...
function M.add_user_message(user_message)
    tokens = utils.get_token_count(user_message)
    --print(tokens)
    ...
    --db.ChatDB.chats:update_tokens(current_chat.id, tokens)
...
function M.add_message_to_chat(user_message)
    ...
    local function on_complete(err)
        ...
        --for _, chunk in ipairs(message_test_log) do
            --vim.api.nvim_buf_set_lines(buf, -1, -1, false, {chunk[1] .. ' ' .. chunk[2]})
        --end
    end
